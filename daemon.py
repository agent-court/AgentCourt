...
def run_listener_cycle():
    try:
        ...
        logging.info(f"Scanning Base Mainnet Escrow contract ({agent.ESCROW_ADDRESS}). Total tasks on-chain: {task_count}")

        for task_id in range(1, task_count + 1):
            task = agent.escrow_contract.functions.tasks(task_id).call()
            # Solidity Enum: 0 = Created, 1 = Submitted, 2 = Resolved
            client, worker, amount, spec, deliverable, created_at, status = task

            if status == 1:
                logging.info(f"[DISPUTE/SUBMISSION DETECTED] Task #{task_id}")
                ...
                if tx_hash:
                    logging.info(f"[SETTLED] Task #{task_id} on-chain! Tx: https://basescan.org/tx/{tx_hash}")
                    ...
                    logging.error(f"[ERROR] Failed settlement transaction on Task #{task_id}.")
...
def main():
    ...
    logging.info(f"Polling Rate   : Every {POLL_INTERVAL_SECONDS}s")
    logging.info("========================================================")
