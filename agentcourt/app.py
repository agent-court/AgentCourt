...
DEFAULT_TREASURY = os.getenv("TREASURY_ADDRESS", "0xc2eC09e66052927D28574DF4AdF0095fe3C425B6")
...
try:
    ...
    treasury_addr = DEFAULT_TREASURY
...
with st.sidebar:
    ...
    st.text_input("Treasury Wallet", treasury_addr, disabled=True)

# Fetch Metrics
try:
    total_tasks = contract.functions.taskCount().call()
    treasury_usdc = usdc.functions.balanceOf(w3.to_checksum_address(treasury_addr)).call() / 1e6
    treasury_eth = w3.from_wei(w3.eth.get_balance(w3.to_checksum_address(treasury_addr)), "ether")
...
with tab1:
    ...
    if total_tasks == 0:
        ...
        try:
            ...
            with r2:
                st.markdown(f"**Spec Hash:** `{t[4]}`")
                st.markdown(f"**Deliverable:** `{t[5] or 'Pending'}`")
